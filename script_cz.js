...
let weatherApi = async function (cityName) {
    try{
        ...
        console.log("Počasí není dostupné, problém: ", error);
        setTimeout( () => {
            resultArray.pop()
...
submitBtn.addEventListener("click", async () => {
    ...
    try{
        ...
        console.log("Počasí není dostupné, problém: ", city, error)
    }
})
...
let renderCities = async() =>{
    ...
    resultArray.forEach(  (city, index) =>{
        ...
        weather.textContent = `Teplota: ${Math.round(city.main.temp)} °C`
        ...
        wind.textContent = `Rychlost větru: ${city.wind.speed} ms`
        ...
        deleteItem.textContent = "Vymazat"
